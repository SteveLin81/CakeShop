...
| `display_name` | `VARCHAR(100) NOT NULL DEFAULT ''` | 會員姓名（B2E 可設定；B2C 註冊時預設等於帳號名） |
...
| PUT | `/api/b2e/users/{id}` | 修改 B2C 會員（姓名 / Email / 密碼） |
...
| `/b2e/admin/users` | `members` | 前台會員管理：新增/編輯/刪除、帳號 / 姓名 / Email 即時搜尋 |
...
| 前台會員管理 | 帳號 / 姓名 / Email 即時搜尋，支援新增（含姓名欄位）/ 修改 / 刪除 |
...
| DTO | 驗證規則 |
|-----|---------|
| `B2cUserCreateRequest` | Username: `[Required][MaxLength(50)][RegularExpression 英數字]`；DisplayName: `[MaxLength(100)]`（可選，空時預設帳號名）；Password: `[Required][MinLength(6)]`；Email: 正規表達式驗證（可選） |
| `B2cUserUpdateRequest` | DisplayName: `[MaxLength(100)]`；Email: 正規表達式驗證（可選）；NewPassword: `[MinLength(6)]`（可選） |
...
- [x] 修正 B2E 商品管理分類欄位空白：`localCategoryName(row)` 改用 `ProductDto` 扁平欄位（`categoryName` / `categoryNameEn` … `categoryNameMs`），不再錯誤存取不存在的 `row.category` 巢狀物件
- [x] 修正 B2E 列表語系切換後資料欄位不更新：所有使用 `locale` 動態顯示的 `el-table`（商品/公告/角色/後台帳號）加入 `:key="locale"` binding，語系切換時強制重建表格以確保 Element Plus CDN scoped slot 正確追蹤響應式依賴
- [x] 修正 B2E 商品管理欄位標頭英文顯示「Name(ZH-TW)」：新增 `product.colName` / `announcement.colContent` 專用欄標頭 i18n 鍵（8 語系），與表單欄位標籤（`product.name` 含語系後綴）分離；Products.cshtml / Announcements.cshtml 列表標頭改用新鍵
- [x] 修正 B2E 後台帳號管理「使用者名稱」欄標頭固定中文：改為 `:label="t('admin.displayName')"` 動態繫結；8 語系新增 `admin.displayName` 鍵（en: Display Name / ja: 表示名 / …）
- [x] B2E 角色權限管理種子資料補齊多語系：DbSetup 的 `b2e_roles` 種子 INSERT 改為 `ON CONFLICT (name) DO UPDATE SET`，補入 admin / 商品部門 / 客服部門的 7 語系名稱與說明；重跑 DbSetup 即可更新既有角色多語系資料
- [x] B2C 前台會員管理新增「姓名」(DisplayName) 欄位：`User` entity / `B2cUserDto` / `B2cUserCreateRequest` / `B2cUserUpdateRequest` 新增 `DisplayName`；`B2cUserManagementService` 建立時若姓名為空則預設帶入帳號名；`AuthService` 的 Register / ForgotPassword / ResetPassword 也同步保留 `DisplayName`（防止 EF Core 全欄更新清空姓名）；`users` 資料表 ALTER TABLE 新增 `display_name VARCHAR(100)` 欄位並回填現有資料；`Users.cshtml` 列表新增姓名欄，表單新增姓名輸入欄位，搜尋支援姓名關鍵字
- [ ] 加入 JWT 標準認證中介層
